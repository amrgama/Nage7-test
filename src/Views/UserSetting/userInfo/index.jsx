...
import { myProfileServices } from "Services";
...
const EditTeacher = () => {
  ...
  const { user,handleOnlyUser } = useContext(UserContext);
  ...
  const handleEdit = async (values) => {
    ...
    try {
      const response = await myProfileServices.updateProfile(data);
      if (response.status === 200) {
        navigate(`${routePath}myProfile`);
        handleOnlyUser(response.data)
      } else {
        reject(response);
      }
    } catch (error) {
  ...
  return (
    ...
    >
      <Grid item xs={12}>
        <Typography variant="h5">{strings.editTeacher}</Typography>
      </Grid>

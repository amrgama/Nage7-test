...
const EditExamModule = () => {
  ...
  const { data } = useQueryService({
    key: ["examModuleServices.getExamModuleById", id],
    fetcher: () => examModuleServices.getExamModuleById(id),
    select: (res) => {
      const {
        firstGrade,
        secondGrade,
    ...
    onSuccess: ({ questions }) => {
      if (!defaultQuestionBank.length) setSelectedQuestion(questions ?? []);
    },
  });
  ...
  useEffect(() => {
    if (activeStep) {
      navigate(`${routePath}exam-modules/edit/${id}?activeStep=${activeStep}`, {
        replace: true,
      });
    }
  }, [activeStep]);
    ...
    <Grid container classes={{ root: classes.wrapper }} spacing={3}>
      ...
      {activeStep === 1 && (
        <ExamDetails
          ...
          defaultValues={data?.examDetails}
        />
      )}
      {activeStep === 2 && (
        <QuestionBank
